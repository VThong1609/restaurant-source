...
import com.dev.restaurant.models.Token;
...
public interface TokenRepository extends JpaRepository<Token, Long> {
    ...
    @Query(value = """
                ...
                WHERE
                        t.token != :token AND
                        t.staff_id IN (
                            SELECT u.id FROM staffs u
                            WHERE u.email = :email
                        )
            """, nativeQuery = true)
    void revokeAllTokenWithoutCurrentTokenWithEmailStaff(@Param("email") String email, @Param("status") int status, @Param("token") String token);
    @Modifying
    @Query(value = """
                UPDATE tokens t
                SET t.revoked = :status
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    void changeRevokeWithEmailStaff(@Param("email") String email, @Param("status") int status);
}
